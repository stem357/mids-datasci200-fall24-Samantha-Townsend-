...
# Generate the features and weights matrices as per the assignment instructions
np.random.seed(25)
features = np.random.randint(0, 100, 10000).reshape(1000, 10)
weights = np.array([0.24540611, 0.31467517, -0.07656614, -0.16161533, -0.09064962,
                    -0.00315615, 0.09609595, 0.2517064, 0.01100181, -0.38629211])
...
def get_shape(ar):
    ...
    Args:
        ar (numpy.ndarray): The input matrix.
        
    Returns:
        tuple: The shape of the matrix as (rows, columns).
    """
    return ar.shape
...
def check_min_max(ar):
    """
    Returns the minimum and maximum values in the input matrix.
    
    Args:
        ar (numpy.ndarray): The input matrix.
        
    Returns:
        tuple: (minimum_value, maximum_value) of the matrix.
    """
...
def count_nan(ar):
    """
    Returns the count of NaN (Not a Number) values in the input matrix.
    
    Args:
        ar (numpy.ndarray): The input matrix.
        
    Returns:
        int: The count of NaN values in the matrix.
    """
...
def get_median(ar):
    """
    Returns the median value for each column in the input matrix.
    
    Args:
        ar (numpy.ndarray): The input matrix.
        
    Returns:
        numpy.ndarray: Array of median values for each column.
    """
...
def feat_minimum(ar):
    min_val = ar.min()
    row_indices = np.where(np.any(ar == min_val, axis=1))[0]
    return row_indices
...
def feat_maximum(ar, feat_num):
    max_val = ar[:, feat_num].max()
    row_indices = np.where(ar[:, feat_num] == max_val)[0]
    return row_indices
...
def classify(np_score):
    ...
    prediction = np.where(np_score >= mean_score, 1, 0)
    return prediction
...
def num_correct(np_truth, np_pred):
    # True Negatives: cases where both truth and prediction are 0
    correct_zero_prediction = np.sum((np_truth == 0) & (np_pred == 0))
    
    # True Positives: cases where both truth and prediction are 1
    correct_one_prediction = np.sum((np_truth == 1) & (np_pred == 1))
